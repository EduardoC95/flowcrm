...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Editar', href: `/people/${props.person.id}/edit` },
];

const form = useForm({
    name: props.person.name,
    entity_id: props.person.entity_id ? String(props.person.entity_id) : '',
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <section>
                <h1 class="text-2xl font-semibold tracking-tight">Editar pessoa</h1>
                <p class="text-sm text-muted-foreground">{{ person.name }}</p>
            </section>
            ...
            >
                ...
                <div class="flex gap-2">
                    ...
                    <Button as-child type="button" variant="outline">
                        <Link :href="`/people/${person.id}`">Cancelar</Link>
                    </Button>
                </div>
            </form>
        </div>
